			...
			optionGroups.forEach(function (group, index) {
				if (hasLabels) {
					if (index != 0) {
						...
						divider.className = selector$1 + '__divider';
					...
					divider.className = selector$1 + '__divider-label';
...
const createWrapper = function (menu) {
	...
	menuWrapper.className = selector$1;
	...
	//add classes
	button.className = selector$1 + '__button';
	buttonLabel.className = selector$1 + '__label';
	buttonCaret.className = selector$1 + '__caret';

	//add content
	buttonLabel.textContent = menu.options[menu.selectedIndex].text;
	if (!menu.options[menu.selectedIndex].hasAttribute('value')) {
		buttonLabel.classList.add(selector$1 + '__label--placeholder');
	}

	//create the menu container
	optionList = document.createElement('ul');
	optionList.className = selector$1 + '__menu';
...
const createMenuItem = function (menuItem) {
	...
	if (menuItem.hasAttribute('value')) {
		...
		//set classnames
		item.className = selector$1 + '__item';
		itemIcon.className = selector$1 + '__item-icon';
		itemLabel.className = selector$1 + '__item-label';
		...
		if (menuItem.index === selectedItem) {
			item.classList.add(selector$1 + '__item--selected');
...
var displayMenu = function (event) {
	...
	if (this.tagName == 'BUTTON') {
		...
		this.classList.toggle(selector$1 + '__button--active');

		//toggle the menu
		let menu = this.parentNode.querySelector('UL');
		menu.classList.toggle(selector$1 + '__menu--active');
		...
		menuItems.forEach((item) => {
			item.classList.remove(selector$1 + '__item--selected');
		});

		//select item
		this.classList.add(selector$1 + '__item--selected');
		...
		let buttonLabel = button.querySelector('.' + selector$1 + '__label');
		buttonLabel.textContent = this.textContent;
		buttonLabel.classList.remove(selector$1 + '__label--placeholder');

		button.classList.toggle(selector$1 + '__button--active');
...
var isOutside = function (event) {
	let selectMenus = document.querySelectorAll('select.' + selector$1);
	...
	selectMenus.forEach((select) => {
		...
		if (menu.classList.contains(selector$1 + '__menu--active')) {
			let clickInside = menuWrapper.contains(event.target);
			if (!clickInside) {
				menu.classList.remove(selector$1 + '__menu--active');
				button.classList.remove(selector$1 + '__button--active');
			}
		}
	});
};
...
const selectMenu = {
	init: function () {
		...
		elements = document.querySelectorAll('.' + selector$1);
